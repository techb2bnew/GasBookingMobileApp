...
const orderSlice = createSlice({
  ...
  reducers: {
    ...
    handleOrderAssigned: (state, action) => {
      const {orderId, orderNumber, agentId, agentName, assignedAgentId} =
        action.payload;
      const order = state.orders.find(
        order =>
          order.id === orderId ||
      ...
      if (order) {
        ...
        order.lastUpdated = new Date().toISOString();
        console.log(
          '✅ Order assigned in Redux:',
          order.orderNumber,
          agentName,
        );
      } else {
        console.log(
          '⚠️ Order not found in Redux for assignment:',
          orderId,
          orderNumber,
        );
      }
    ...
    handleOrderDelivered: (state, action) => {
      const {orderId, orderNumber, deliveryProof, paymentReceived} =
        action.payload;
      const order = state.orders.find(
        order =>
          order.id === orderId ||
      ...
      if (order) {
        ...
        console.log(
          '⚠️ Order not found in Redux for delivery:',
          orderId,
          orderNumber,
        );
      }
    ...
    reorder: (state, action) => {
      const originalOrder = state.orders.find(
